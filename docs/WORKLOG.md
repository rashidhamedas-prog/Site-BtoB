...
## 2026-07-18 — رفع ریدایرکت اشتباه `/admin` به داشبورد مشتری

### خلاصه
- اگر کاربر با نقش غیر‌ادمین (مثلاً مشتری لاگین‌شده) به `/admin` می‌رفت، middleware او را به `/portal/dashboard` می‌فرستاد
- اصلاح: ریدایرکت به `/admin/login` + رد کردن لاگین مشتری روی فرم ادمین
...
### تست
- تأیید روی پروداکشن قبل از فیکس: با کوکی `CUSTOMER` مسیر `/admin` → `/portal/dashboard`
- بدون کوکی: `/admin` → `/admin/login` (صحیح بود)
