...
## ⚠️ الان یک کار فوری در Cloudflare باقی مانده

nic درست است (NSهای Cloudflare). ولی در DNS هنوز **رکورد تکراری وبزی** هست و ترافیک را می‌دزدد.

الان dig می‌گوید دامنه هنوز به `31.214.255.53` (Webzi) می‌رود.

### همین الان در Cloudflare → DNS → Records حذف کن:

| Type | Name | Content | وضعیت | کار |
|------|------|---------|-------|-----|
| A | `poshaktaranom.ir` | `31.214.255.53` | Proxied نارنجی ⚠️ | **حذف** |
| A | `www` | `31.214.255.53` | Proxied نارنجی ⚠️ | **حذف** |

### این‌ها را نگه دار:

| Type | Name | Content | Proxy |
|------|------|---------|-------|
| A | `poshaktaranom.ir` | `5.75.200.102` | DNS only (خاکستری) |
| A | `www` | `5.75.200.102` | DNS only (خاکستری) |

`ftp` / `mail` / `pop` / `smtp` اگر هنوز ایمیل روی وبزی است موقتاً بماند؛ بعداً جدا منتقل کن.

بعد از حذف، چک کن:
```text
nslookup poshaktaranom.ir 1.1.1.1
```
باید فقط `5.75.200.102` ببینی.

سپس روی سرور (خودمان هم اجرا می‌کنیم وقتی DNS درست شد):
```bash
bash /opt/taranom/scripts/enable-ir-ssl.sh
```
