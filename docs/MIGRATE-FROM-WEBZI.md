...
## وضعیت (۲۰۲۶-۰۷-۲۳) — انجام شد

- nic → NS کلودفلر ✓
- Cloudflare A برای `@` و `www` → `5.75.200.102` ✓ (رکورد وبزی حذف شد)
- Let’s Encrypt برای `poshaktaranom.ir` + `www` صادر و روی nginx فعال شد ✓
- از سرور: `https://www.poshaktaranom.ir` → کانال RETAIL ✓

در Cloudflare Proxy را **DNS only (خاکستری)** نگه دار. اگر نارنجی کردی، SSL/TLS را روی **Full (strict)** بگذار.

اگر روی سیستم خودت هنوز Webzi دیدی، کش DNS مودم/اینترنت است. با این چک کن:
```text
nslookup www.poshaktaranom.ir 1.1.1.1
```
باید `5.75.200.102` باشد. DNS ویندوز را `1.1.1.1` بگذار یا مودم را یک‌بار خاموش/روشن کن.
